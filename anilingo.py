...
english_translation = random_lyric['english']
korean_translation = random_lyric['korean']
original_tokens = random_lyric['tokens'].split(',')
...
font_path = "Noto Sans Mono CJK JP Regular.otf"
font_size = 32
font = pygame.font.Font(font_path, font_size)
font_path = "gulim.ttc"
font_size = 32
kfont = pygame.font.Font(font_path, font_size)
...
def reset_game():
    global random_lyric, original_lyric, english_translation, korean_translation, tokens, selected_tokens
    random_lyric = random.choice(lyrics_data)
    original_lyric = random_lyric['original']
    english_translation = random_lyric['english']
    korean_translation = random_lyric['korean']
    tokens = random_lyric['tokens'].split(',')
    random.shuffle(tokens)
    selected_tokens = []

def draw_token_boxes(tokens):
    global start_x, y
...
while running:
            ...
            for i, box_rect in enumerate(token_boxes):
                if box_rect.collidepoint(event.pos):
                    if i in selected_tokens:
                        selected_tokens.remove(i)  # Deselect the token
                    else:
                        selected_tokens.append(i)  # Select the token
                    needs_redraw = True
                    break
    ...
    if needs_redraw:
        ...
        if len(selected_tokens) == len(tokens):
            ...
            #print(selected_token_values, original_tokens)
            if selected_token_values == original_tokens:
                result_text = "Correct!"
            ...
            result_text_rect = result_text_surface.get_rect(center=(screen_width // 2, 150))
            screen.blit(result_text_surface, result_text_rect)
        pygame.display.flip()
        needs_redraw = False

    if event.type == pygame.MOUSEBUTTONDOWN:
        if next_question_button.collidepoint(event.pos) and len(selected_tokens) == len(tokens) and selected_token_values == original_tokens:
            reset_game()
            needs_redraw = True
